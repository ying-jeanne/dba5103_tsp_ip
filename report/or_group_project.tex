...
\begin{titlepage}
    ...
    {\large
    \uncertain{Team Member 1 Name} \\
    \uncertain{Team Member 2 Name} \\
    \uncertain{Team Member 3 Name} \\
    \uncertain{Team Member 4 Name} \\
    }
    \vspace{1cm}
    {\large
    ...
    \vspace{0.3cm}
    \href{https://github.com/qiyazhao062-a11y/DBA5103_TSP_Data}{Link to code repository}
    }
    \vfill
\end{titlepage}
\thispagestyle{fancy}
...
%% ============================================
\section*{Abstract}
\uncertain{Ash: will write out this part after all parts are finalised}
...
During implementation, we observed that the formulations became computationally expensive for instances with 20 cities. In several test runs, the solver required more than ten hours to obtain the optimal solution for many of the 10 instances in a given scenario.

To ensure the feasibility of experimentation, we imposed a time limit of 300 seconds (5 minutes) on the MTZ model. This threshold was selected based on the average of preliminary tests from smaller city numbers (i.e., 15 and 18) that captured the typical solving time before excessive computational escalation. When the solver reached the time limit, it returned the best $z$ and corresponding $w$ available at that time. As a result, the computed integrality gaps under the time-limited runs are approximate.
...
\begin{tabular}{lccc}
\toprule
\textbf{Formulation} & \textbf{F-Stat} & \textbf{p-value} & \textbf{Sig.?} \\
\midrule
DFJ & 1.90 & 0.148 & No \\
...
\end{enumerate}
Each corresponding gap of approximately 74\% for the former two combinations, and 44\% for the last.
...
\small
\begin{tabular}{p{2.2cm}p{2.5cm}p{3.2cm}p{4.5cm}}
\toprule
\textbf{CV} & \textbf{Structure} & \textbf{Recommended} & \textbf{Expected Gap} \\
\midrule
Low (0.4--0.46) & Grid, Random Euclidean & AP, MTZ, GG & 0\%--20\% \\
\addlinespace
Medium (0.6) & Clustered & DFJ (best), GG & $\approx$0\% (DFJ), $\approx$44\% (GG) \\
\addlinespace
Med--High (0.62--0.65) & Hub-and-Spoke & DFJ (best) & $\approx$0\% (DFJ), 12\%--16\% (others) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Limitations:}
\begin{itemize}
    \item Computational constraints limited the problem size to $n=15$ for most benchmarks, especially for the assignment problem.
